"""Append and replay Corpus Factory lifecycle evidence."""
...
class CorpusEventService:
    def __init__(self, store: EventStorePort) -> None:
        self._store = store
        self._actor = ActorRef(actor_type=ActorType.SYSTEM, actor_id="corpus-factory")
    ...
    async def append(self, stream_id: UUID, payload: EventPayload, *, correlation_id: UUID) -> int:
        ...
        envelope = create_event_envelope(
            ...
            actor=self._actor,
            source_component="corpus-factory",
    ...
    async def replay(self, stream_id: UUID) -> tuple[EventPayload, ...]:
        ...
        models = {model.event_type: model for model in CORPUS_EVENT_MODELS}
        return tuple(
