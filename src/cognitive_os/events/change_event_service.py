"""Append and replay controlled-change lifecycle evidence."""
...
from .change_events import CHANGE_EVENT_MODELS


class ChangeEventService:
    def __init__(self, store: EventStorePort) -> None:
        self._store = store
        self._actor = ActorRef(actor_type=ActorType.SYSTEM, actor_id="controlled-changes")
    ...
    async def append(self, stream_id: UUID, payload: EventPayload, *, correlation_id: UUID) -> int:
        ...
        envelope = create_event_envelope(
            ...
            actor=self._actor,
            source_component="controlled-changes",
    ...
    async def replay(self, stream_id: UUID) -> tuple[EventPayload, ...]:
        models = {model.event_type: model for model in CHANGE_EVENT_MODELS}
