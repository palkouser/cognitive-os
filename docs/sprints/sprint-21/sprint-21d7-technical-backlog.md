...
| 10, 11, 13 | ✅ **met in W3** | final A and B opened once: 30 groups and 120 outcomes each, after W3-F1 replaced four unencodable carried bodies; 45 changed decisions |
...
| 14, 16 | ✅ **met in W3** | +0.3833 absolute and 0.7419 relative error reduction; final A +0.300, final B +0.467, each against its own re-derived baseline |
| 15, 18–23 | ✅ **met in W3** | bootstrap [0.233333, 0.533333] excluding zero; 0 safety moves; 0 retention loss; 120/60 metamorphic decisions with bound 0.036754; 45 shadow-would-change and 0 executed; the artifact re-ranks all 100 groups identically; all 18 reason codes reached |
| 24 | met | inherited under the renewed ruling; three voiding identities recomputed at gate close |
| 25–27 | ✅ **met in W3** | the canary manifest is the catalogue's own hash and 5 groups routed; 5 of 5 proposals verified by the hidden suite; the kill switch answers `lifecycle_not_active` on the next call; four processes and two database restarts; a human approval naming the exact assessment, revision and lineage, with three refusals executed |
| 28 | ✅ **met in W4** | 46 of 46 release-matrix rows, 0 skipped, 0 structural findings |
| 29 | ✅ **met in W4** | #229 squash-merged into protected `main` as `3f5d7379…`, exact-head CI run `31476479587` 30 of 30, annotated tag `sprint-21-learning-baseline` created after it and never moved, all read back from the remote with zero findings |
...
| **W3** ✅ **closed — activated on the canary subset** | Only on a pass: v3 artifact bound to the new conformal point; loader, resolver, sequencer against the real artifact; **open the carried roles** — final A and B, 120 outcomes each over 30 groups; paired bootstrap; safety and retention; promotion metamorphic inside the admission budget; shadow; canary manifest, human approval, kill switch; activation, restart survival, deliberate rollback. **Done, with one finding: W3-F1, the carried final roles could not be encoded under the frozen contract — four D2-authored bodies predate the reflection ban, and five sprints of unchanged digests never asked. Audited under S21D7-038, four replacement groups authored, the frozen 30/120 counts kept.** Every listed condition met; no threshold moved | 10, 11, 13–16, 18–23, 25–27 |
| **W4** ✅ **closed — Gate L2 passes, 29 of 29** | Full release matrix; protected release, exact-head CI, annotated tag, remote verification; gate assessment (`scripts/gate_assessment_d7.py`, counts-derived verdict, stop-hash-bound rows on a stop); Gate D1 6, 7 from final surface evidence and 15 by the renewed inheritance; report and handoff. **Done, with one finding: W4-F1, the W1 seal still claimed three carried roles byte-identical to D6's after W3 repaired two of them under authority. Rebound rather than edited, and the drift stop narrowed rather than removed.** Condition 28 met at 46 of 46 rows; condition 29 closed on the gate owner's protected merge, its exact-head CI and the annotated tag. Gate D1 6, 7 and 15 closed. **Sprint 22A unblocked** | 28, 29 |
