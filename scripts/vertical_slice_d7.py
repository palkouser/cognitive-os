#!/usr/bin/env python3
"""S21D7-024. One fixture group from task package to admission, on a group no role holds.

Before D7 runs 100 groups and 500 containers, it runs one, end to end, on a group that is in no
D7 catalogue: `d5_fixture.render_duration`. If the spine is broken, this is where it should
break — on a group nobody is allowed to count. Section 6.1 requires the slice to spend no
bar-setting case, certification case, final member or canary member, and that is not asserted
here: the group is checked against the sealed S21D7-023 bundle before anything runs, including
against the two roles that are held beside the catalogues rather than in them.

*Why D5's fixture rather than a fresh one.* The fixture's only contract is to be outside every
role, and this one is: it is in none of D7's nine, which the seal check below proves rather than
claims. It carries D7's own campaign namespace, seed and candidate identities, so nothing it
writes collides with D5's or D6's run of it, and it lives in D7's isolated store. Authoring a
fresh fixture would cost a near-clone check against 626 released groups and buy the slice nothing
...
6. the **v3 relational assembly**, a **seven-channel fit** over it, and split-conformal
   admission over two disjoint halves of the fixture's own decisions at the carried alpha;
7. the **v3 artifact** written from that direction, reloaded from its own canonical bytes, and
   rebuilt into a ranker of the *new* class;
...
What is new against D6's slice is step 6's representation, and it is the whole reason D7 exists.
D6 fitted 390 channels here. D7 assembles seven — the six sealed v2 scalars, taken out of the
seal by name, and the containment share derived from the baseline module and the four candidate
sources — and fits the same rule over them. Everything else is D6's slice with a different
representation underneath, deliberately: a spine proof that changed the parts it is not testing
would not be a proof of the parts it is.

Section 5.1 names two seams to probe hard, and both are executed rather than described.
`relational_numbers` must refuse a sealed record whose scalar names have drifted, because that
is the one way a fit can silently read the wrong numbers under the right names. And a
**pure-deletion group** — one whose candidates add no lines at all — must score every share zero
and fall back to the caller's frozen order, because the share is a relation over added lines and
a group that adds none has no containment to measure.

One property of the fold is D7's own: the containment share is a relation over the group, so
holding a candidate out changes what the remaining shares mean. They are recomputed over the
remainder rather than sliced out of the whole group's, because a share computed against a
sibling that is no longer there is a number about a group that does not exist.

The conformal arithmetic is unchanged from D6 and so is its honesty clause. On a conformal half
this small the finite-sample rank ceil((1-alpha)*(m+1)) exceeds the number of wrong margins for
every m a four-candidate group can produce, so at the carried alpha the fixture gets **no
quantile and admits nothing**. Because a bar that does not exist cannot bind an artifact, step 7
derives a second, clearly-labelled fixture bar at an alpha the tiny sample can support, uses only
that one to bind, and records both. Neither is D7's operating point: that one is derived once, in
W2, from the sealed bar-setting half.
...
Storage is the isolated D7 pair from S21D7-002 (`COGOS_DATABASE_URL`, `COGOS_ARTIFACT_ROOT`).
No predecessor store is opened — D5's and D6's least of all, because D7 reads its envelope from
one and its whole bar-setting half from the other.

    set -a && . ./.env.s21d7.local && set +a
    UV_CACHE_DIR=.cache/uv uv run python scripts/vertical_slice_d7.py \
...
def _role_boundary(group_name: str, template_id: str) -> dict[str, Any]:
    """Ask the sealed S21D7-023 bundle whether this group belongs to anything. It must not.
    ...
    try:
        ...
        bundle = seal_d7_corpus(provisional=True)
    roles = {
    ...
    # And the ninth role, for the same reason the conformal half is asked separately: it is held
    # beside the catalogues, not in them. It is spent and read by nothing, but a slice that
    # borrowed one of its groups would still be borrowing evidence a ruling declined to reuse.
    roles["spent_calibration_d5"] = sorted(
        item.repository_group for item in bundle.spent_calibration.groups
    )
    ...
    return {
        ...
        "authored_by": (
            "S21D5-024, as D5's fixture; outside every D7 role and re-run under D7 identities "
            "in D7's own store"
        ),
        "why": (
...
async def _run_slice(
    *,
    ...
    campaign_id = uuid5(D7_CAMPAIGN_NAMESPACE, "d6:vertical-slice")
    ...
    dataset_report = {
        ...
        "why_those_scans_cannot_pass_here": {
            ...
            "where_it_is_asked_for_real": (
                "S21D7-030 scans D5's conformal matrix against D6's certification matrix, which "
    ...
    # 6. the v3 assembly, the seven-channel fit, and split-conformal admission ----------
    #
    # This is D7's step 6, and it is the whole reason the sprint exists. D6 fitted 390 channels
    # here; D7 assembles seven — the six sealed v2 scalars, read out of the seal by name, and
    # the containment share derived from the group package — and fits the same rule over them.
    # Everything below this assembly is D6's slice with a different representation underneath,
    # deliberately: a spine proof that changed the parts it is not testing would not be a proof
    # of the parts it is.
    baseline_source = reality_candidates.baseline_source(task_manifest)
    baseline_order = tuple(str(slot.candidate_id) for slot in ordered)
    relational = relational_numbers(
        {key: seal.record_for(UUID(key)).values for key in baseline_order},
        baseline_source=baseline_source,
        sources_by_candidate={key: sources[key] for key in baseline_order},
    )
    shares = containment_shares(baseline_source, {key: sources[key] for key in baseline_order})
    ...
    assembly_report = {
        ...
        "scalars_read_from": "the sealed v2 record, by name",
        "derived_channel": REPAIR_CONTAINMENT_CHANNEL,
        "baseline_source_sha256": _digest(baseline_source),
        "candidates": [
            {
                "candidate_id": key,
                "channels": [
                    [name, float(value)]
                    for name, value in zip(FITTED_RELATIONAL_CHANNELS, relational[key], strict=True)
                ],
                "accepted": accepted_of[key],
            }
            for key in baseline_order
        ],
        "share_by_candidate": {key: shares[key] for key in baseline_order},
        "containment_ordering_alone": list(
            containment_ordering(
                baseline_source,
                {key: sources[key] for key in baseline_order},
                baseline_order=baseline_order,
            )
        ),
        "distinct_relational_vectors": len({relational[key] for key in baseline_order}),
        "shares_inside_the_unit_interval": all(0.0 <= shares[key] <= 1.0 for key in baseline_order),
        "embedding_read_by_any_channel": False,
    }

    # §5.1's two named seams, probed here rather than asserted. The first is the one the
    # campaign also executes: the assembly takes its scalars by name, so a record whose names
    # drifted must fail rather than feed the direction numbers under the wrong names.
    assembly_report["refusals"] = [
        _refusal(
            "assemble a relational vector from drifted scalar names",
            lambda: relational_numbers(
                {
                    key: tuple(
                        ("drifted_" + name, value)
                        for name, value in seal.record_for(UUID(key)).values
                    )
                    for key in baseline_order
                },
                baseline_source=baseline_source,
        ...
        _refusal(
            "assemble a relational vector from a single candidate",
            lambda: relational_numbers(
                {baseline_order[0]: seal.record_for(UUID(baseline_order[0])).values},
                baseline_source=baseline_source,
                sources_by_candidate={baseline_order[0]: sources[baseline_order[0]]},
            ),
        ),
    ]

    # The second seam: a group whose candidates add nothing. The share is a relation over added
    # lines, so on a pure-deletion group there is nothing to be contained in — every share is
    # zero and the ordering has to fall back to the frozen baseline order rather than to a
    # candidate id or to whatever `sorted` does with ties. Driven on the fixture's own baseline
    # standing in for all four candidates, which is the smallest thing that is genuinely a
    # pure-deletion group rather than a description of one.
    deletion_sources = {key: baseline_source for key in baseline_order}
    deletion_shares = containment_shares(baseline_source, deletion_sources)
    assembly_report["pure_deletion_group"] = {
        "every_share_is_zero": set(deletion_shares.values()) == {0.0},
        "ordering_is_the_frozen_baseline_order": (
            containment_ordering(baseline_source, deletion_sources, baseline_order=baseline_order)
            == baseline_order
        ),
        "reversed_baseline_order_is_honoured_too": (
            containment_ordering(
                baseline_source, deletion_sources, baseline_order=tuple(reversed(baseline_order))
            )
            == tuple(reversed(baseline_order))
        ),
        "why_this_is_not_a_failure": (
            "the share is a relation over the lines a repair adds, so a group that adds none "
            "has no containment to measure. §6 of the backlog prices this: the signal degrades "
            "to a consensus prior rather than failing, and the tie-break is the caller's frozen "
            "order everywhere in this programme"
        ),
    }

    group_of = RelationalGroup(
        group=group.repository_group,
        order=baseline_order,
        numbers=relational,
        accepted={key: accepted_of[key] for key in baseline_order},
    )
    model = fit_containment_direction([group_of], regularization=FIXTURE_REGULARIZATION)

    # The scored set. A D7 decision is a *within-group ordering*, so a per-candidate score would
    ...
    #
    # One thing about the fold is D7's own and worth stating: the containment share is a
    # *relation over the group*, so holding a candidate out does not merely drop a row — it
    # changes what the remaining candidates' shares mean. The shares are recomputed over the
    # remainder rather than sliced out of the whole group's, because a share computed against a
    # sibling that is no longer there is a number about a group that does not exist.
    ...
    for row in matrix.rows:
        ...
        rest = tuple(key for key in baseline_order if key != held_out)
        if not any(accepted_of[key] for key in rest) or all(accepted_of[key] for key in rest):
            ...
            held_out_reports.append(
                {"held_out": held_out, "fitted": False, "why": "one-sided remainder"}
            )
            continue
        fold_numbers = relational_numbers(
            {key: seal.record_for(UUID(key)).values for key in rest},
            baseline_source=baseline_source,
            sources_by_candidate={key: sources[key] for key in rest},
        )
        fold = fit_containment_direction(
            [
                RelationalGroup(
                    group=f"{group.repository_group}:without:{held_out}",
                    order=rest,
                    numbers=fold_numbers,
                    accepted={key: accepted_of[key] for key in rest},
                )
            ],
            regularization=FIXTURE_REGULARIZATION,
        )
        order = rest
        ranked = ContainmentContrastiveRanker(fold).rank(
            {key: fold_numbers[key] for key in order}, baseline_order=order
        )
    ...
    ranking = ranker.rank(relational, baseline_order=baseline_order)
    ...
    ranking_report = {
        ...
        "relational_assembly": assembly_report,
        ...
        "direction": {
            ...
            "why_that_is_said_rather_than_hidden": (
                ...
                "with -- and D6 fits none at all: S21D7-034 reads the sealed 720 direction"
            ),
        },
        "admission": {
            ...
            "why": (
                "D6's bar is derived once, at S21D7-034, from the sealed conformal half of 400 "
            ...
            "why_the_pre_registered_alpha_cannot_produce_a_quantile_here": {
                "arithmetic": (
                    ...
                    f"ceil((1-{D7_ALPHA})*(m+1)) is {point.quantile_rank}, which exceeds them, so "
    ...
    descriptor_hash = _digest(f"d7-vertical-slice-descriptor:{dataset.content_hash}")
    ...
    reloaded_ranking = reloaded.rank(relational, baseline_order=baseline_order)
    ...
    restored_ranking = restored_ranker.rank(relational, baseline_order=baseline_order)
...
async def _run(output: Path, model: Path) -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="cogos-d7-slice-") as scratch:
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=EVIDENCE / "sprint-21d7-vertical-slice.json")
