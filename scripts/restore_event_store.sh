...
change_counts_json="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT json_build_object('experiments',(SELECT count(*) FROM cognitive_os.change_experiments),'revisions',(SELECT count(*) FROM cognitive_os.change_experiment_revisions),'isolations',(SELECT count(*) FROM cognitive_os.change_isolation_manifests),'candidates',(SELECT count(*) FROM cognitive_os.change_candidates),'candidate_artifacts',(SELECT count(*) FROM cognitive_os.change_candidate_artifacts),'evaluations',(SELECT count(*) FROM cognitive_os.change_evaluation_runs),'comparisons',(SELECT count(*) FROM cognitive_os.change_regression_comparisons),'assessments',(SELECT count(*) FROM cognitive_os.change_promotion_assessments),'promotions',(SELECT count(*) FROM cognitive_os.change_promotions),'rollbacks',(SELECT count(*) FROM cognitive_os.change_rollbacks),'accesses',(SELECT count(*) FROM cognitive_os.change_accesses))")"
change_integrity="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT NOT EXISTS (SELECT 1 FROM cognitive_os.change_experiment_revisions r LEFT JOIN cognitive_os.change_experiments e USING (experiment_id) WHERE e.experiment_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.change_experiments e LEFT JOIN cognitive_os.change_experiment_revisions r ON r.experiment_id=e.experiment_id AND r.revision=e.current_revision WHERE r.experiment_id IS NULL OR r.content_hash<>e.current_content_hash) AND NOT EXISTS (SELECT 1 FROM cognitive_os.change_promotions p LEFT JOIN cognitive_os.change_experiments e USING (experiment_id) WHERE e.experiment_id IS NULL)")"
change_history_sha256="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT record_kind, identity, content_hash FROM (SELECT 'experiment' record_kind, experiment_id::text identity, current_content_hash content_hash FROM cognitive_os.change_experiments UNION ALL SELECT 'revision', experiment_id::text || ':' || revision, content_hash FROM cognitive_os.change_experiment_revisions UNION ALL SELECT record_kind, isolation_record_id::text, content_hash FROM cognitive_os.change_isolation_manifests UNION ALL SELECT record_kind, candidate_id::text, content_hash FROM cognitive_os.change_candidates UNION ALL SELECT record_kind, candidate_artifact_id::text, content_hash FROM cognitive_os.change_candidate_artifacts UNION ALL SELECT record_kind, evaluation_run_id::text, content_hash FROM cognitive_os.change_evaluation_runs UNION ALL SELECT record_kind, comparison_id::text, content_hash FROM cognitive_os.change_regression_comparisons UNION ALL SELECT record_kind, assessment_id::text, content_hash FROM cognitive_os.change_promotion_assessments UNION ALL SELECT record_kind, promotion_record_id::text, content_hash FROM cognitive_os.change_promotions UNION ALL SELECT record_kind, rollback_id::text, content_hash FROM cognitive_os.change_rollbacks UNION ALL SELECT record_kind, access_id::text, content_hash FROM cognitive_os.change_accesses) history ORDER BY record_kind, identity" | sha256sum | awk '{print $1}')"
domain_counts_json="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT json_build_object('pilot_runs',(SELECT count(*) FROM cognitive_os.domain_pilot_runs),'problem_references',(SELECT count(*) FROM cognitive_os.domain_problem_references),'derivation_references',(SELECT count(*) FROM cognitive_os.domain_derivation_references),'verification_results',(SELECT count(*) FROM cognitive_os.domain_verification_results),'accesses',(SELECT count(*) FROM cognitive_os.domain_accesses),'transfer_experiments',(SELECT count(*) FROM cognitive_os.domain_transfer_experiments),'transfer_results',(SELECT count(*) FROM cognitive_os.domain_transfer_results))")"
domain_integrity="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT NOT EXISTS (SELECT 1 FROM cognitive_os.domain_problem_references p LEFT JOIN cognitive_os.domain_pilot_runs run USING (run_id) WHERE run.run_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.domain_derivation_references d LEFT JOIN cognitive_os.domain_pilot_runs run USING (run_id) WHERE run.run_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.domain_verification_results v LEFT JOIN cognitive_os.domain_pilot_runs run USING (run_id) WHERE run.run_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.domain_accesses a LEFT JOIN cognitive_os.domain_pilot_runs run USING (run_id) WHERE run.run_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.domain_transfer_results r LEFT JOIN cognitive_os.domain_transfer_experiments e USING (experiment_id) WHERE e.experiment_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.domain_transfer_results WHERE hard_gate_failed AND disposition='positive_transfer')")"
domain_history_sha256="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT kind, identity, content_hash FROM (SELECT 'run' kind, run_id::text identity, problem_hash content_hash FROM cognitive_os.domain_pilot_runs UNION ALL SELECT 'problem', problem_reference_id::text, content_hash FROM cognitive_os.domain_problem_references UNION ALL SELECT 'derivation', derivation_reference_id::text, content_hash FROM cognitive_os.domain_derivation_references UNION ALL SELECT 'verification', verification_result_id::text, content_hash FROM cognitive_os.domain_verification_results UNION ALL SELECT 'access', access_id::text, content_hash FROM cognitive_os.domain_accesses UNION ALL SELECT 'transfer_experiment', experiment_id::text, content_hash FROM cognitive_os.domain_transfer_experiments UNION ALL SELECT 'transfer_result', experiment_id::text, content_hash FROM cognitive_os.domain_transfer_results) history ORDER BY kind, identity" | sha256sum | awk '{print $1}')"
learned_counts_json="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT json_build_object('components',(SELECT count(*) FROM cognitive_os.learned_components),'revisions',(SELECT count(*) FROM cognitive_os.learned_component_revisions),'datasets',(SELECT count(*) FROM cognitive_os.learned_datasets),'artifacts',(SELECT count(*) FROM cognitive_os.learned_artifacts),'evidence_records',(SELECT count(*) FROM cognitive_os.learned_evidence_records),'observations',(SELECT count(*) FROM cognitive_os.learned_observations),'approvals',(SELECT count(*) FROM cognitive_os.learned_activation_approvals),'activations',(SELECT count(*) FROM cognitive_os.learned_activation_history),'accesses',(SELECT count(*) FROM cognitive_os.learned_accesses))")"
learned_integrity="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT NOT EXISTS (SELECT 1 FROM cognitive_os.learned_components c LEFT JOIN cognitive_os.learned_component_revisions r ON r.component_id=c.component_id WHERE r.component_id IS NULL) AND NOT EXISTS (SELECT 1 FROM (SELECT surface FROM cognitive_os.learned_components WHERE current_state='active' GROUP BY surface HAVING count(*)>1) s) AND NOT EXISTS (SELECT 1 FROM cognitive_os.learned_activation_history h LEFT JOIN cognitive_os.learned_activation_approvals a ON a.approval_id=h.approval_id WHERE h.action='activation' AND (a.approval_id IS NULL OR a.content_hash IS DISTINCT FROM h.approval_hash)) AND NOT EXISTS (SELECT 1 FROM cognitive_os.learned_artifacts l LEFT JOIN cognitive_os.artifacts a ON a.artifact_id=l.artifact_id WHERE a.artifact_id IS NULL)")"
learned_history_sha256="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT kind, identity, revision, content_hash FROM (SELECT 'component' kind, component_id identity, current_revision revision, content_hash FROM cognitive_os.learned_components UNION ALL SELECT 'revision', component_id, revision, content_hash FROM cognitive_os.learned_component_revisions UNION ALL SELECT 'dataset', dataset_id::text, revision, content_hash FROM cognitive_os.learned_datasets UNION ALL SELECT 'lineage', lineage_id::text, 1, content_hash FROM cognitive_os.learned_artifacts UNION ALL SELECT 'evidence', evidence_id::text, 1, content_hash FROM cognitive_os.learned_evidence_records UNION ALL SELECT 'observation', observation_id::text, 1, content_hash FROM cognitive_os.learned_observations UNION ALL SELECT 'approval', approval_id::text, component_revision, content_hash FROM cognitive_os.learned_activation_approvals UNION ALL SELECT 'activation', receipt_id::text, component_revision, content_hash FROM cognitive_os.learned_activation_history UNION ALL SELECT 'access', access_id::text, 1, content_hash FROM cognitive_os.learned_accesses) history ORDER BY kind, identity, revision" | sha256sum | awk '{print $1}')"
provider_output_counts_json="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT json_build_object('outputs',(SELECT count(DISTINCT provider_output_id) FROM cognitive_os.provider_output_records),'revisions',(SELECT count(*) FROM cognitive_os.provider_output_records),'retained_artifacts',(SELECT count(*) FROM cognitive_os.provider_output_records WHERE response_artifact_id IS NOT NULL))")"
provider_output_history_sha256="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT provider_output_id, revision, content_hash FROM cognitive_os.provider_output_records ORDER BY provider_output_id, revision" | sha256sum | awk '{print $1}')"
provider_output_integrity="$(psql "$COGOS_RESTORE_DATABASE_URL" -Atqc "SELECT NOT EXISTS (SELECT 1 FROM cognitive_os.provider_output_records GROUP BY provider_output_id HAVING count(*) <> max(revision) OR min(revision) <> 1) AND NOT EXISTS (SELECT 1 FROM cognitive_os.provider_output_records record LEFT JOIN cognitive_os.artifacts artifact ON artifact.artifact_id = record.response_artifact_id WHERE record.response_artifact_id IS NOT NULL AND artifact.artifact_id IS NULL) AND NOT EXISTS (SELECT 1 FROM cognitive_os.provider_output_records record LEFT JOIN cognitive_os.events event ON event.event_id = record.completed_event_id WHERE event.event_id IS NULL)")"
uv run python - "$manifest" "$semantic_observation_count" "$semantic_claim_count" "$semantic_revision_count" "$semantic_evidence_count" "$semantic_relation_count" "$semantic_contradiction_count" "$wiki_page_count" "$wiki_revision_count" "$semantic_history_sha256" "$semantic_as_of_sha256" "$skill_count" "$skill_revision_count" "$skill_history_sha256" "$strategy_count" "$strategy_revision_count" "$strategy_edge_count" "$strategy_selection_count" "$strategy_outcome_count" "$strategy_access_count" "$strategy_history_sha256" "$experience_compilation_count" "$experience_source_count" "$experience_candidate_count" "$experience_decision_count" "$experience_access_count" "$experience_history_sha256" "$corpus_source_count" "$corpus_item_count" "$corpus_route_count" "$corpus_manifest_count" "$corpus_export_count" "$corpus_access_count" "$corpus_history_sha256" "$routing_profile_count" "$routing_profile_revision_count" "$routing_policy_count" "$routing_policy_revision_count" "$routing_observation_count" "$routing_decision_count" "$routing_outcome_count" "$routing_statistics_count" "$routing_experiment_count" "$routing_access_count" "$routing_history_sha256" "$weakness_counts_json" "$weakness_history_sha256" "$proposal_counts_json" "$proposal_history_sha256" "$change_counts_json" "$change_history_sha256" "$domain_counts_json" "$domain_history_sha256" "$learned_counts_json" "$learned_integrity" "$learned_history_sha256" "$provider_output_counts_json" "$provider_output_integrity" "$provider_output_history_sha256" <<'PY'
import json
import sys
...
change_counts = json.loads(sys.argv[50])
change_history_hash = sys.argv[51]
domain_counts = json.loads(sys.argv[52])
domain_history_hash = sys.argv[53]
learned_counts = json.loads(sys.argv[54])
learned_integrity = sys.argv[55]
learned_history_hash = sys.argv[56]
provider_output_counts = json.loads(sys.argv[57])
provider_output_integrity = sys.argv[58]
provider_output_history_hash = sys.argv[59]
...
if manifest.get("change_counts", {}) != change_counts:
    raise SystemExit("restored controlled-change counts do not match the backup manifest")
if manifest.get("change_history_sha256") != change_history_hash:
    raise SystemExit("restored controlled-change history differs from the backup manifest")
if manifest.get("domain_counts", {}) != domain_counts:
    raise SystemExit("restored domain counts do not match the backup manifest")
if manifest.get("learned_counts", {}) != learned_counts:
    raise SystemExit("restored learned counts do not match the backup manifest")
if manifest.get("provider_output_counts", {}) != provider_output_counts:
    raise SystemExit("restored provider-output counts do not match the backup manifest")
if provider_output_integrity != "t":
    raise SystemExit(
        "restored provider-output governance failed its integrity check: a revision chain "
        "has a gap, or a retained artifact or completed event is missing"
    )
if manifest.get("provider_output_history_sha256") != provider_output_history_hash:
    raise SystemExit("restored provider-output history hash does not match the backup manifest")
if learned_integrity != "t":
    raise SystemExit("restored learned evidence failed its structural integrity checks")
if manifest.get("learned_history_sha256") != learned_history_hash:
    raise SystemExit("restored learned history differs from the backup manifest")
if manifest.get("domain_history_sha256") != domain_history_hash:
    raise SystemExit("restored domain history differs from the backup manifest")
PY
[[ -n "$revision" && "$event_count" =~ ^[0-9]+$ && "$artifact_count" =~ ^[0-9]+$ && "$memory_count" =~ ^[0-9]+$ && "$memory_integrity" == "t" && "$semantic_integrity" == "t" && "$semantic_lineage_integrity" == "t" && "$skill_integrity" == "t" && "$strategy_integrity" == "t" && "$experience_integrity" == "t" && "$corpus_integrity" == "t" && "$routing_integrity" == "t" && "$weakness_integrity" == "t" && "$proposal_integrity" == "t" && "$change_integrity" == "t" && "$domain_integrity" == "t" ]]
