#!/usr/bin/env python
"""Sprint 21D5 W7 operations: provisioning, backup, restart, restore, and the failure matrix.

S21D5-082, -083 and -084 in one command, because they are one question asked three times. 082
asks whether the authorities are the ones revision 5 declared; 083 asks whether the evidence
...
would be exercised against a store the second had proved nothing about.
    ...
    scripts/operations_d5.py --output docs/sprints/sprint-21/evidence/sprint-21d5-operations.json

What this command writes, and what it must never write
------------------------------------------------------

It writes to the D5 backup root, to the D5 restore database, and to a scratch directory. It
writes nothing to the D5 evidence pair and nothing at all to the six predecessor pairs -- one
more than D4 had, and the extra one is the store D4 itself wrote. Every fingerprint is taken
...
The order is the finding, not a preference
------------------------------------------

The backup runs before anything is damaged. D4-W0-F1 lost 1,076 committed observations because
every backup was taken after the erasure, and D4-W7-F1 recovered from the same class of accident
only because W7's own backup step had already run three minutes earlier. Nothing in this file
reorders those two steps.

The two answers, and what that changes here
-------------------------------------------

D5 stopped on one branch and passed on the other: S21D5-035 selected no candidate under §3.3
step 5, and the retrieval holdout cleared both floors. So the restore assertion is still the
sharper one -- what must come back is exactly the stopped state, and an absence is the easiest
thing for a restore to get wrong in the direction that looks fine -- while the retrieval classes
are now checked against a record that *passed*, which is the direction a reopened alternative
would be easiest to hide in.

The matrix inherits D4's twenty-two cases, adapted to D5's documents, and adds five this sprint
is the first to be able to run: the sealed direction rehashing to what S21D5-032 recorded and
refusing a single nudged weight, a certificate naming a hypothesis class no loader implements, a
sealed contract naming one, a stop kind outside the four §3.3 published, and a second derivation
of the operating point that does not reproduce the first. All five are refusals that only exist
because D5 fitted something; none of them opens a task the continuation record closed.
"""
...
from cognitive_os.domain.promotion_payload import PromotionGateOutcome
...
from cognitive_os.learning.pairwise_contrastive import PairwiseContrastiveModel
...
D5_ENV_FILE = REPOSITORY / ".env.s21d5.local"

DATA_ROOT = Path("/home/palkouser/projekt/cognitive-os-data")
#: The six stores this sprint must not write to. Fingerprinted before and after everything.
#: D4's own store joins the list D4 kept, which is the whole difference between the two waves
#: here: every sprint inherits one more predecessor than the last, and the newest one is the
#: one an operator is most likely to still have exported.
INHERITED = (
    "artifacts",
    "artifacts-s21c3",
    "artifacts-s21d1",
...
D5_STORE = "artifacts-s21d5"

#: The surface D5's correction experiment would have used. The credential-free lifecycle smoke
...
#: The artifact the loader cases damage is still D3's committed contract fixture. D5 fitted a
#: direction but S21D5-051 -- the item that would bind a derived threshold into a v3 artifact --
#: is one of the twenty-six the continuation record closed, so building a v3 artifact here would
#: be W4's work done in W7 against a record that says it was not done. What 084 asks of the
#: loader is whether damage arriving through an operations path is refused, and that question is
#: about the loader. What D5 *can* ask of its own fitted bytes it asks separately, of the sealed
#: direction, in `_direction_cases`.
...
def _run(argv: Sequence[str], *, env: Mapping[str, str] | None = None) -> tuple[int, str]:
    ...
    `COGOS_POSTGRES_ENV_FILE` is set on every call: S21D5-W0-F1 measured that the shell scripts
    re-source their own environment file under `set -a` over whatever the caller exported, so
    pointing them at the D5 file is the only way a D5 command can be sure it is not operating on
    a predecessor store.
    """
    completed = subprocess.run(  # nosec B603 - fixed argv list, shell=False
        ...
        check=False,
        cwd=REPOSITORY,
        env={**os.environ, "COGOS_POSTGRES_ENV_FILE": str(D5_ENV_FILE), **(env or {})},
    )
...
def _fingerprints() -> dict[str, dict[str, object]]:
    ...
    for name in (*INHERITED, D5_STORE):
        root = DATA_ROOT / name
...
#: Every hashed row the D5 evidence rests on, rolled into one digest. Counts alone would not
...
async def _provisioning() -> dict[str, Any]:
    """The authorities are the ones revision 5 declared, and no wider.
    ...
    if not migrations.is_dir():
        # D4-W7-A2. The first version of this pointed at a directory that does not exist, so
        # the list was empty and "no migration 0016" passed by looking nowhere. A check that
    ...
    return {
        "database": name,
        "database_is_isolated": "s21d5" in name,
...
def _env_file_referenced() -> dict[str, Any]:
    """Every D5 operations example must name `COGOS_POSTGRES_ENV_FILE`.

    Checked against the file rather than asserted, because the reason it matters is what
    S21D5-W0-F1 measured: the shell scripts re-source their own environment under `set -a`, so
    an example that omits it runs against whatever the last export left behind.
    """
    return {
        "operations_script_sets_it": "COGOS_POSTGRES_ENV_FILE"
        in Path(__file__).read_text(encoding="utf-8"),
        "env_file": D5_ENV_FILE.name,
        "env_file_present": D5_ENV_FILE.is_file(),
    }
...
async def _backup(report: dict[str, Any]) -> Path:
    """Back the D5 pair up with the repository's own script, and record what it produced."""
    ...
    if body.get("database_name") != expected:
        raise SystemExit(
            ...
            "the D5 environment was overridden and this backup is not D5's"
        )
...
async def _restart_container() -> str:
    ...
    for _ in range(60):
        ...
        try:
            ...
            return f"{container} restarted; the D5 database answered again"
        except Exception:  # the store is not back yet, which is the point of waiting
            await asyncio.sleep(1)
        finally:
            await engine.dispose()
    raise SystemExit(f"{container} restarted but the D5 database never answered again")
...
async def _prove_recovery(report: dict[str, Any], scratch: Path) -> tuple[Path, list[str]]:
    """S21D5-083: back up, restart, restore, and prove the two copies are the same evidence."""
    ...
    # The stop, restored exactly. An absence is the easiest thing for a restore to get wrong in
    # the direction that looks fine, so it is asserted rather than assumed.
    report["restore"]["stopped_state"] = {
        ...
        "note": (
            ...
            "skill.selection; it is not a D5 correction component and is expected to restore"
        ),
    }
...
def _observed_blobs(root: Path, declared: Sequence[str]) -> dict[str, str | None]:
    ...
    Keyed by what the store *says* it holds rather than by what is on disk: a map built from
    the files present reports a store one blob smaller as perfectly clean, and a partial
...
def _artifact_cases() -> list[dict[str, object]]:
...
def _direction_from(data: bytes) -> PairwiseContrastiveModel:
    """The stored direction, parsed the way `learner_selection_d5._model_from_bytes` writes it.

    Eight lines rather than a by-path import of a thousand-line campaign script. The canonical
    form that decides the hash lives in `PairwiseContrastiveModel`, which is imported, so the
    only thing repeated here is the field mapping.
    """
...
async def _sealed_directions(
    restore_url: str, extracted: Path
) -> dict[int, tuple[PairwiseContrastiveModel, str]]:
    """The two directions S21D5-032 fitted, read back out of the *restored* copy.

    The committed record carries the hashes and the channel count, not the 390 weights, so the
    only place the direction itself exists is the store — which makes this the sharper question
    anyway: not "does the record agree with itself" but "did the bytes survive the move".
    """
    from sqlalchemy import text as sql

    record = json.loads((EVIDENCE / "sprint-21d5-direction-fit.json").read_text(encoding="utf-8"))
    engine = _engine(restore_url)
    try:
        async with engine.connect() as connection:
            addresses = {
                ...
                for row in await connection.execute(
                    sql("SELECT artifact_id::text, content_hash FROM cognitive_os.artifacts")
                )
            }
    finally:
        await engine.dispose()

    directions: dict[int, tuple[PairwiseContrastiveModel, str]] = {}
    for entry in record["models"]:
        address = addresses.get(str(entry["artifact_id"]))
        if address is None:
            raise SystemExit(f"the restored copy has no artifact {entry['artifact_id']}")
        blob = extracted / "sha256" / address[:2] / address
        directions[int(entry["volume_rows"])] = (
            _direction_from(blob.read_bytes()),
            str(entry["model_hash"]),
        )
    return directions


def _direction_cases(
    directions: Mapping[int, tuple[PairwiseContrastiveModel, str]],
) -> list[dict[str, object]]:
    """D5's own fitted bytes: the sealed direction, and the same direction one weight off.

    The first row is not damage — it is the control the damage row needs. A rehash that refuses
    a nudged weight proves nothing unless the unnudged one is accepted, and the record has to
    show both or a reader cannot tell a working check from a check that refuses everything.
    """
    rows: list[dict[str, object]] = []
    for volume in sorted(directions):
        model, sealed = directions[volume]
        nudged = PairwiseContrastiveModel(
            encoder_version=model.encoder_version,
            feature_names=model.feature_names,
            weights=(model.weights[0] + 1e-9, *model.weights[1:]),
        ...
        rows.append(
            _row(
                f"sealed_direction_{volume}_rows",
                "none: the direction read back out of the restored copy",
                "the restored bytes rehash to the hash S21D5-032 sealed, so the damage row "
                "below means something",
                {
                    "failed_closed": model.content_hash() == sealed,
                    "reason": f"rehashed to {model.content_hash()[:12]}, sealed {sealed[:12]}",
                    "is_a_control_not_a_damage_case": True,
                },
            )
        )
        rows.append(
            _row(
                f"nudged_direction_{volume}_rows",
                f"one weight of the {volume}-row direction moved by 1e-9",
                "the canonical form refuses: a direction is its weights, to twelve digits",
                {
                    "failed_closed": nudged.content_hash() != sealed,
                    "reason": f"nudged rehashes to {nudged.content_hash()[:12]}",
                },
            )
        )
    return rows
...
def _matrix(
    ...
    absent = d5_integrity(EVIDENCE, blob_hashes=_observed_blobs(extracted, declared))
    rows.append(
        _row(
            "missing_blob",
    ...
    rows.extend(_artifact_cases())
    rows.extend(_direction_cases(directions))

    # Evidence damage, each seeded into its own copy and decided by the released report.
    for name, damage, expected, mutate in (
        ...
        (
            ...
            "a fusion variant opened after the holdout returned a *passing* result",
            "retrieval_one_read fails: a passing read followed by tuning is the same failure, "
            "and the easier one to excuse",
            lambda root: _edit(
                root,
        ...
        (
            "dropped_matrix_scan",
            "a snapshot record keeping three scan results and calling them all of them",
            "matrix_embedding_scans fails: the required set is named by the class, not read "
            "out of the record it is checking",
            lambda root: _edit(
                root,
                "sprint-21d5-snapshots.json",
        ...
        (
            ...
            "the sealed contract naming a hypothesis class no loader implements",
            "feature_schema fails: S21D5-037's question, asked of the contract",
            lambda root: _edit(
                root,
        ...
        (
            "stop_kind_outside_the_published_four",
            "a continuation record ending at a stop kind §3.3 never published",
            "lifecycle fails: an ending chosen after the measurement is not an ending",
            lambda root: _edit(
                root,
        ...
        (
            "dependent_item_with_no_reason",
            "one of the twenty-six not-opened items with its reason removed",
            "lifecycle fails: a dependent bound to a stop has to say what the stop cost it",
            lambda root: _edit(
                root,
                "sprint-21d5-continuation.json",
        ...
        (
            ...
            lambda root: _edit(
                root,
                "sprint-21d5-operating-point.json",
                lambda d: d.update(
                    json.loads(
                        json.dumps(d).replace(
                            '"independent_decisions": 100', '"independent_decisions": 10000'
                        )
                    )
                ),
            ),
        ...
        (
            ...
            lambda root: _edit(
                root,
                "sprint-21d5-baseline-ladder.json",
                lambda d: d.update(
                    json.loads(
                        json.dumps(d).replace(
    ...
    rows.extend(_threshold_cases())

    # Retrieval policy substitution: an unfrozen resource policy hash.
    rows.append(
        _row(
    ...
    # Judgement substitution: searchable text that names the label it is scored against.
    leaks = judgement_leaks(
        {"pair:failed": "coding\nd5r_boundary_collections:thing\nobservation status=completed\n"},
    ...
    # Store isolation: a predecessor pair checked against a digest that is not its own.
    rows.append(
        _row(
            "inherited_store_isolation",
            "the D4 pair checked against a fingerprint that is not its own",
            "the isolation class names the store and refuses",
            {
                "failed_closed": (
                    d5_integrity(
                        EVIDENCE, predecessor_fingerprints={"sprint_21d4": "0" * 64}
                    ).failed
                    != ()
                ),
...
def _scored(count: int, *, wrong: int = 0) -> list[ScoredDecision]:
    return [
        ScoredDecision(
            decision_id=f"d{index}",
            feature_hash=_digest(f"case:{index}"),
            score=Decimal("0.9") - Decimal(index) / 100,
            answered=True,
            correct=index >= wrong,
        )
...
def _threshold_cases() -> list[dict[str, object]]:
    """The two refusals that guard the threshold itself, neither of which reads a file."""
    sealed = derive_zero_error_point(
        _scored(8),
        split="calibration",
        calibration_source_hash=_digest("calibration-source"),
        derived_at=datetime(2026, 8, 9, tzinfo=UTC),
    )
    return [
        _row(
        ...
        _row(
            ...
            "a second derivation, over different decisions, handed the first one back",
            "the single-shot rule refuses: a point derived twice was searched, not derived",
            _refusal(
                lambda: derive_zero_error_point(
                    _scored(8, wrong=1),
                    split="calibration",
        ...
        _row(
            ...
            "a condition-20 certificate naming a hypothesis class no loader implements",
            "the gate refuses: S21D5-037 made the class a checked field, not a label",
            _refusal(
                lambda: condition_20_gate(
                    outcome=PromotionGateOutcome.PASSED,
                    evidence_hash=_digest("a certificate that does not exist"),
                    detail="120 decisions, zero confident errors",
                    census=DecisionCensusV4.from_feature_hashes(
                        [_digest(f"ood:{index}") for index in range(120)]
                    ),
                    calibration_certificate_hash=_digest("calibration-certificate"),
                    hypothesis_class="pairwise-contrastive-v9",
                )
            ),
        ),
    ]
...
async def _execute(output: Path) -> int:
    ...
    scratch = Path(os.environ.get("COGOS_SCRATCH_ROOT", DATA_ROOT / "scratch-s21d5")) / (
        "w7-operations"
    )
    if scratch.exists():
    ...
    report: dict[str, Any] = {
        ...
        "sprint": "21D5",
        "wave": "W7",
        "items": ["S21D5-082", "S21D5-083", "S21D5-084", "S21D5-085"],
        "purpose": (
            "Verify D5 provisioning, prove the evidence survives backup, restart and an "
            "isolated restore, and demonstrate that damage to the moved copy is refused."
        ),
        "artifact_under_test": ARTIFACT_UNDER_TEST,
        "started_at": started,
        "pre_registration_sha256": _file_digest(EVIDENCE / "sprint-21d5-pre-registration.json"),
    ...
    directions = await _sealed_directions(_require("COGOS_RESTORE_DATABASE_URL"), extracted)
    report["corruption_matrix"] = _matrix(scratch, extracted, declared, directions)
    report["matrix_shape"] = _matrix_shape(report["corruption_matrix"])
    ...
    report["isolation"] = {
        ...
        "d5_pair_unchanged": before[D5_STORE] == after[D5_STORE],
        ...
        json.dumps(
            {
                ...
                "matrix_damage_cases": report["matrix_shape"]["damage_cases"],
                "matrix_controls": report["matrix_shape"]["controls"],
...
def _matrix_shape(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """How many of the table's rows are damage, and how many are the controls damage needs.

    Two rows in the table break nothing: they assert that the restored direction still rehashes
    to what S21D5-032 sealed. Counting them as damage cases would inflate the number by two and
    would let a reader believe a control had refused something. So they are named apart, and the
    exit status still requires every row — control and damage alike — to hold.
    """
    controls = [
        str(row["case"])
        for row in rows
        if bool(dict(row["observed"]).get("is_a_control_not_a_damage_case"))
    ]
    return {
        "rows": len(rows),
        "damage_cases": len(rows) - len(controls),
        "controls": controls,
        "why_a_control_is_in_the_table": (
            "a rehash that refuses a nudged weight proves nothing unless the unnudged one is "
            "accepted; the control is what makes the damage row readable"
        ),
    }
...
def _ci_coverage() -> dict[str, object]:
    """S21D5-085: which D5 surfaces the credential-free lanes actually cover."""
    ...
    covered = {
        ...
        "d5_evidence_schemas": "d5-integrity" in workflow,
        "d5_integrity_classes": "test_d5_integrity.py" in workflow,
        "d5_cli_boundary": "test_d5_cli_boundary.py" in workflow,
        "hypothesis_class": "test_d5_condition_20_hypothesis_class.py" in workflow,
        "truncation_fence": "test_truncation_fence.py" in workflow,
...
def _findings(report: Mapping[str, Any]) -> list[str]:
    ...
    if not provisioning["no_migration_0016"]:
        findings.append("a migration 0016 exists, which D5 forbids")
    ...
    if not restore["stopped_state"]["no_correction_component_was_registered"]:
        findings.append("a component exists on the correction surface, which D5 never opened")
    open_cases = [
        row["case"]
        for row in report["corruption_matrix"]
    ...
    if report["ci_coverage"]["uncovered"]:
        uncovered = report["ci_coverage"]["uncovered"]
        findings.append(f"credential-free lanes do not cover: {uncovered}")
    return findings


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", type=Path, default=EVIDENCE / "sprint-21d5-operations.json")
