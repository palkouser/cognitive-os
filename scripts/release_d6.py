#!/usr/bin/env python
"""S21D6-095: the release, recorded from the remote rather than from prose.

    scripts/release_d6.py --pull-request <n> [--output docs/.../sprint-21d6-release.json]
...
D6's outcome is a typed stop and the gate does not pass, so a `sprint-21-learning-baseline` in
existence would contradict every other record in the sprint. Its absence is asserted rather than
assumed.

*And it does not hard-code the pull request.* D5 carried its PR number as a module constant, which
is fine once the release has happened and wrong every moment before it. `--pull-request` is
required, so a run against the wrong release fails at the remote rather than by describing one
...
TAG = "sprint-21d6-evidence-baseline"
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=EVIDENCE / "sprint-21d6-release.json")
    parser.add_argument(
        "--pull-request",
        type=int,
        required=True,
    ...
    continuation = _read("sprint-21d6-continuation.json")
    gate = _read("sprint-21d6-gate-l2.json")
    selection = _read("sprint-21d6-learner-selection.json")
    ruling = _read("sprint-21d6-condition-24-ruling.json")
    isolation = _read("sprint-21d6-authority-isolation-after.json")
    ...
    report: dict[str, Any] = {
        ...
        "pre_registration_sha256": hashlib.sha256(
            (EVIDENCE / "sprint-21d6-pre-registration.json").read_bytes()
        ).hexdigest(),
        "outcome": "negative; one typed stop and no candidate",
        ...
        "retrieval_branch": {
            ...
            "ruling_sha256": hashlib.sha256(
                (EVIDENCE / "sprint-21d6-condition-24-ruling.json").read_bytes()
            ).hexdigest(),
            ...
            "note": (
                "D6 ran no retrieval branch. Condition 24 and Gate D1 condition 15 are inherited "
        ...
        "component_state": {
            ...
            "note": (
                "S21D6-035 recorded a null under §3.4 step 2; nothing was registered, approved "
                "or activated on experience.correction_ranking. Source: "
                "sprint-21d6-continuation.json, stop kind "
                f"{continuation['decision']['stop_kind']!r}, "
                f"{continuation['not_opened']['count']} pieces of dependent work bound to one "
                "hash"
            ),
        },
        ...
        "documents": {
            "report": "sprint-21d6-report.md",
            "assessment": "gate-l2-d5-assessment.md",
            "handoff": "sprint-21d6-handoff.md",
            "execution_log": "sprint-21d6-execution.md",
        },
    }
