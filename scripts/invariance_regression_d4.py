#!/usr/bin/env python3
"""S21D4-038. The invariance-regression sample: forty transformations that add no decision.

S21D4-032 sealed `invariance_independent_decisions: 0` on the strength of the W1 erratum -- a
transformation repeats its source group's fitted feature vector, so it repeats the decision.
That was a claim. This is where it is executed.

Forty cases over the declared twenty-group sample, two per group, and three things have to hold
at once or the seal was wrong about what it sealed:
...
*The first action does not move.* With no fitted ranker in force the first action is the head of
the baseline order, so what is checked here is that the transformation did not reorder what the
system acts on first. The ranker-dependent form of this check belongs to S21D4-039's grid.

And one control, because the first two are also what a normaliser that erased everything would
produce: seeded semantic mutations must change the canonical representation. A canonicaliser
that maps every program to the same bytes would pass invariance perfectly and be worthless.
...
observation is projected, and no dataset grows. `entered_any_dataset` is false and stays false.
    ...
    UV_CACHE_DIR=.cache/uv uv run python scripts/invariance_regression_d4.py \\
...
OUTPUT = EVIDENCE / "sprint-21d4-invariance-regression.json"
...
async def _embed(provider: Any, sources: list[str]) -> tuple[tuple[float, ...], ...]:
    ...
    Deliberately not batched. The frozen MiniLM is batch-composition dependent: padding a
    short sequence alongside a long one moves its vector by about 1e-7, which is invisible to
    a cosine and fatal to a hash. Batching here would make this regression measure which batch
    a body landed in rather than whether the transformation changed it -- the first run did
    exactly that and reported sixty-two changed vectors, seven of them for a case that does not
...
def _cases() -> tuple[tuple[_Case, ...], list[str], Any]:
    ...
    by_group = {spec.repository_group: spec for spec in D4_CALIBRATION_SPECS}
    built: list[_Case] = []
...
def _run_one(job: tuple[str, str, str, str]) -> tuple[str, bool]:
    ...
    with tempfile.TemporaryDirectory(prefix="cogos-d4-invariance-") as directory:
        root = Path(directory)
...
def _clean_labels() -> tuple[dict[str, dict[int, bool]], dict[str, int]]:
    ...
    Indexed by *variant*, not by slot position. `recipe_binding` shuffles which authored
    variant each recipe carries, per template, precisely so that no recipe is correct more
    often than chance -- so slot zero is not variant zero, and comparing them by index compares
    two different candidates. The first run of this script did, and reported ninety-two label
    changes that were nothing but that mismatch.
    """
    campaign = json.loads(CALIBRATION_CAMPAIGN.read_text(encoding="utf-8"))
    catalogue = seal_d4_corpus().catalogues[CorrectionPartition.CALIBRATION]
...
async def _batch_dependence_probe(provider: Any) -> dict[str, Any]:
    """Measure W2-D9 rather than assert it: one text, two batches, one comparison."""
    target = "def f(a):\n    return a + 1\n"
    padding = (
        "def g("
...
async def _run(output: Path, model: Path) -> int:
    provider, _ = _embedding_provider(model)
    cases, inapplicable, submanifest = _cases()
    sample = invariance_sample_groups(seal_d4_corpus().catalogues[CorrectionPartition.CALIBRATION])
    ...
    # The bounds are the fitting partition's, reused. Refitting them on the transformed set
    # would let the transformation move the encoder rather than be measured against it.
    rows = [
        raw_numeric_row_v2(
            feature_input_v2(
    ...
    evidence = _seal(
        {
            ...
            "purpose": (
                "execute the claim S21D4-032 sealed: a transformation repeats its source "
                "group's fitted feature vector, so it adds no independent decision"
            ),
            "submanifest": {
                ...
                "cases_by_name": {
                    name: sum(1 for case in cases if case.case_name == name) for name in D4_CASES
                },
                "source_groups": len({case.group for case in cases}),
                "sample_groups_named": list(sample),
                "sample_matches_the_seal": sorted({case.group for case in cases}) == sorted(sample),
            },
            "chronology": {
            ...
            "independence": {
                ...
                "reading": (
                    ...
                    "its clean counterpart, so the forty transformed decisions repeat twenty "
                    "clean ones and add none. This is the number S21D4-032 sealed as zero and "
                    "the property the regression tests"
                ),
            },
            "verifier": {
            ...
            "first_action": {
                ...
                "reading": (
                    ...
                    "its canonical identity. The ranker-dependent form belongs to S21D4-039"
                ),
            },
            ...
            "findings": [
                {
                    "id": "W2-D9",
                    "found_by": "the first run of this regression, which reported 62 changed "
                    "vectors including 7 for a case that does not touch the candidate source",
                    "detail": (
                        "the frozen MiniLM is batch-composition dependent. The same text "
                        "embedded on its own and embedded alongside a longer sequence differ, "
                        "because the shorter one is padded to the batch's length. The "
                        "difference is around 1e-7 -- invisible to a cosine, fatal to a hash"
                    ),
                    "measured": batching,
                    "consequence": (
                        "a sealed v2 feature vector is reproducible from its candidate source, "
                        "the fitting bounds *and the exact batch it was encoded in*. The first "
                        "two alone are not enough, and any check that re-encodes under "
                        "different batching will disagree with the seal"
                    ),
                    "what_it_does_not_invalidate": (
                        "the seals themselves, which are records of the bytes that were "
                        "actually encoded, and every scan and threshold that compares vectors "
                        "by distance rather than by hash"
                    ),
                    "how_this_record_avoids_it": (
                        "every source here is embedded window by window on its own, so a "
                        "difference between a clean and a transformed vector can only come "
                        "from the transformation"
                    ),
                },
                {
                    "id": "W2-D10",
                    "found_by": "reading S21D4-034's re-encode proof while investigating W2-D9",
                    "detail": (
                        "sprint-21d4-feature-seals.json reports "
                        "`reencodes_identically_from_source_alone: true` for both partitions. "
                        "The check behind it re-ran the sealing function over the same "
                        "PendingFeatureV2 records, which already carry their embeddings, so it "
                        "never re-embedded anything. It proves the sealing function is "
                        "deterministic given identical inputs, which is worth knowing and is "
                        "not what the field name says"
                    ),
                    "correction": (
                        "read that field as 're-seals identically from the same encoded "
                        "records'. Given W2-D9 the stronger claim its name makes is false: "
                        "re-embedding under different batching would not reproduce the seal"
                    ),
