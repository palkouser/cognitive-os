"""Report canonical read-only Memory Plane health diagnostics."""
...
from cognitive_os.infrastructure.memory.postgres.health import PostgresMemoryHealthService
...
async def run() -> int:
    ...
    try:
        report = await PostgresMemoryHealthService(engine).check()
    finally:
