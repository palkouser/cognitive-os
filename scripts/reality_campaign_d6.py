#!/usr/bin/env python3
"""S21D6-025 and S21D6-026. Seal every feature before any container, then run one campaign.
...
is not measuring.
    ...
    UV_CACHE_DIR=.cache/uv uv run python scripts/reality_campaign_d6.py \
        --model /home/palkouser/projekt/cognitive-os-data/models/all-MiniLM-L6-v2
    UV_CACHE_DIR=.cache/uv uv run python scripts/reality_campaign_d6.py --stage execute
    UV_CACHE_DIR=.cache/uv uv run python scripts/reality_campaign_d6.py --stage snapshot
...
#: S21D6-026 names one item for both partitions, and each run writes its own record, or the
#: second would overwrite the first campaign's.
CAMPAIGN_RECORD = {
    CorrectionPartition.CALIBRATION: EVIDENCE / "sprint-21d6-certification-campaign.json",
}

SNAPSHOT_RECORD = EVIDENCE / "sprint-21d6-snapshots.json"
VERTICAL_SLICE = EVIDENCE / "sprint-21d6-vertical-slice.json"

ACTOR = "reality-campaign-d6"
AUTHORITY = "S21D6-026"
...
class _Partition:
    ...
    def __init__(self, partition: CorrectionPartition, groups: tuple[CatalogueGroup, ...]) -> None:
        ...
        self.groups = groups
        self.campaign_id = uuid5(D6_CAMPAIGN_NAMESPACE, f"d6:{partition.value}")
...
async def _stage_seal(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="cogos-d6-seal-") as scratch:
            for name in _ORDER:
...
async def _stage_execute(
    ...
    *,
    ...
    catalogue = seal_d6_corpus(provisional=provisional).catalogues[partition]
    ...
    try:
        ...
        if seal.content_hash != row["feature_seal_hash"]:
            raise SystemExit(
                ...
                f"{row['feature_seal_hash']} S21D6-025 recorded"
            )

        campaign_id = uuid5(D6_CAMPAIGN_NAMESPACE, f"d6:{partition.value}")
        prepared_of = {}
        with tempfile.TemporaryDirectory(prefix="cogos-d6-run-") as scratch:
...
async def _stage_snapshot(output: Path) -> int:
    ...
    try:
        ...
        for name in _ORDER:
            ...
            seal = SealedFeatureRecordSetV2.model_validate_json(seal_bytes.decode())
            if seal.content_hash != row["feature_seal_hash"]:
                raise SystemExit(f"{name.value}: the stored seal is not the one S21D6-025 recorded")
