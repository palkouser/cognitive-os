...
def _runtime(args: argparse.Namespace) -> Runtime:
    ...
    events = SemanticMemoryEventService(PostgresEventStore(engine, build_default_event_catalog()))
    service = SemanticMemoryService(
        repository,
    ...
    return Runtime(engine, repository, memory, resolver, service)
...
async def _execute(runtime: Runtime, args: argparse.Namespace) -> None:
    ...
    if command == ("observation", "create"):
        ...
        for span in observation.source_spans:
            await runtime.resolver.validate_span(span)
        _dump(
        ...
        _dump(await repository.list_observations(source_id=args.source_id, limit=args.limit))
    ...
    elif command == ("claim", "history") or args.group == "timeline":
        ...
        decision = (
            ClaimPromotionDecision.model_validate(_json_file(args.decision_input))
        ...
        _dump(
            await runtime.service.transition_contradiction(
                revision, expected_revision=args.expected_revision
            )
        )
...
async def _wiki(runtime: Runtime, args: argparse.Namespace) -> None:
    ...
    if args.action == "verify":
        revision = await repository.get_wiki_revision(args.page_id, args.revision)
        _dump(
            {
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    for action in ("promote", "dispute", "supersede", "retract"):
        ...
        command.add_argument("--decision-input", type=Path)
