#!/usr/bin/env python3
"""S21D6-020 and S21D6-022: execute the D6 certification corpus and prove it is separated.
...
Nothing here reads an outcome, a candidate score or a verifier label from any D5 or D6 role.
...
a metric. In particular this tool never reads a conformal margin: the bar belongs to W2.
    ...
    UV_CACHE_DIR=.cache/uv uv run python scripts/corpus_d6.py
    UV_CACHE_DIR=.cache/uv uv run python scripts/corpus_d6.py --groups d6-boundary-window-starts
...
def _job(job: tuple[str, str, str, str, str, str]) -> tuple[str, str, str, bool]:
    ...
    with tempfile.TemporaryDirectory(prefix="cogos-d6-authoring-") as directory:
        root = Path(directory)
...
def _execute(groups: tuple[str, ...]) -> dict[str, Any]:
    selected = [
        spec for spec in D6_CERTIFICATION_SPECS if not groups or spec.repository_group in groups
    ]
...
def _separation() -> dict[str, Any]:
    ...
    d6_groups = {spec.repository_group for spec in D6_CERTIFICATION_SPECS}
    collisions = []
    for pair in pairs:
        ...
        if left_group == right_group:
            continue
        if left_group not in d6_groups and right_group not in d6_groups:
            continue
        collisions.append({"left": pair.left, "right": pair.right, "reason": pair.reason})
    return {
        "bodies_compared": len(sources),
        "d6_groups": len(d6_groups),
        "cross_group_collisions_touching_21d6": collisions,
...
def _search(words: tuple[str, ...]) -> int:
    ...
    released = (
        ...
        *D5_CALIBRATION_SPECS,
        *D5_RETRIEVAL_SPECS,
        # D6's own corpus counts as occupied ground the moment it is authored.
...
def main() -> int:
    ...
    report = {
        "sprint": "21D6",
        "items": ["S21D6-020", "S21D6-022"],
        "certification_groups_authored": len(D6_CERTIFICATION_SPECS),
        "calibration_target": CERTIFICATION_TARGET,
        "shortfall": CERTIFICATION_TARGET - len(D6_CERTIFICATION_SPECS),
        "families": _families(),
        "execution": execution,
