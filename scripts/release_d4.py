#!/usr/bin/env python
"""S21D4-095: the release, recorded from the remote rather than from prose.

    scripts/release_d4.py [--output docs/.../sprint-21d4-release.json]
...
D4's outcome is negative; a `sprint-21-learning-baseline` in existence would contradict every
other record in the sprint, so its absence is asserted rather than assumed.
"""
...
IMPLEMENTATION_PR = 223
GATE_CLOSE_PR_TITLE = "Sprint 21D4: close Gate L2 condition 29 on the verified release handles"
TAG = "sprint-21d4-evidence-baseline"
...
def _findings(report: dict[str, Any]) -> list[str]:
    ...
    if release["success_tag_exists"]:
        findings.append(f"{SUCCESS_TAG} exists, and D4's outcome is negative")
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=EVIDENCE / "sprint-21d4-release.json")
    ...
    checkpoint = _read("sprint-21d4-pre-final-checkpoint.json")
    selection = _read("sprint-21d4-learner-selection.json")
    retrieval = _read("sprint-21d4-retrieval-decision.json")
    operations = _read("sprint-21d4-operations.json")

    report: dict[str, Any] = {
        "schema_version": 1,
        "sprint": "21D4",
        "wave": "W8",
        "items": ["S21D4-094", "S21D4-095"],
        "recorded_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
        "pre_registration_sha256": hashlib.sha256(
            (EVIDENCE / "sprint-21d4-pre-registration.json").read_bytes()
        ).hexdigest(),
        "outcome": "negative",
        "gate_l2": "does not pass",
        "d1_conditions_still_open": [6, 7, 15],
        ...
        "stop_hashes": {
            ...
            "retrieval_holdout": retrieval["stop_hash"],
        },
        "component_state": {
            ...
            "note": (
                "S21D4-039 recorded a null and S21D4-059 refused final access; nothing was "
                "registered, approved or activated on experience.correction_ranking. Source: "
                "sprint-21d4-pre-final-checkpoint.json, authorised "
                f"{checkpoint['decision']['authorised']}"
            ),
        },
        "store_fingerprints_unchanged": operations["isolation"]["predecessor_pairs_unchanged"],
        "documents": {
            "report": "sprint-21d4-report.md",
            "assessment": "gate-l2-d4-assessment.md",
            "handoff": "sprint-21d5-handoff.md",
            "execution_log": "sprint-21d4-execution.md",
        },
    }
