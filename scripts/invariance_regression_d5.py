#!/usr/bin/env python3
"""S21D5-031. The invariance-regression sample: forty transformations that add no decision.

S21D5-023 sealed `invariance_independent_decisions: 0` over twenty of the hundred freshly
authored calibration groups. That is a claim about D5's own corpus, not an inherited one: the
bodies are new, the transformations are seeded differently, and whether a rename survives the
...
observation is projected, and no dataset grows. `entered_any_dataset` is false and stays false.
    ...
    UV_CACHE_DIR=.cache/uv uv run python scripts/invariance_regression_d5.py \\
...
OUTPUT = EVIDENCE / "sprint-21d5-invariance-regression.json"
...
def _cases() -> tuple[tuple[_Case, ...], list[str], Any]:
    ...
    by_group = {spec.repository_group: spec for spec in D5_CALIBRATION_SPECS}
    built: list[_Case] = []
...
def _run_one(job: tuple[str, str, str, str]) -> tuple[str, bool]:
    ...
    with tempfile.TemporaryDirectory(prefix="cogos-d5-invariance-") as directory:
        root = Path(directory)
...
def _clean_labels() -> tuple[dict[str, dict[int, bool]], dict[str, int]]:
    ...
    catalogue = seal_d5_corpus().catalogues[CorrectionPartition.CALIBRATION]
...
async def _batch_dependence_probe(provider: Any) -> dict[str, Any]:
    """Measure W2-D9 on D5's own model copy rather than inherit it: two batches, one text."""
    target = "def f(a):\n    return a + 1\n"
    padding = (
        "def g("
    ...
    evidence = _seal(
        {
            ...
            "purpose": (
                "execute the claim S21D5-023 sealed about D5's own calibration corpus: a "
            ...
            "submanifest": {
                ...
                "cases_by_name": {
                    name: sum(1 for case in cases if case.case_name == name) for name in D5_CASES
                },
                ...
                "sample_is_drawn_from": (
                    "the D5 calibration catalogue — twenty of the hundred groups authored for "
            ...
            "independence": {
                ...
                "reading": (
                    ...
                    f"{len(sample)} clean ones and add none. This is the number S21D5-023 "
            ...
            "first_action": {
                ...
                "ranker_dependent_form": {
                    ...
                    "why_not_a_second_run": (
                        ...
                        "report it as if it were an observation. S21D5-035 reads the two "
            ...
            "batch_composition_dependence": {
                ...
                "why_the_hashes_here_are_not_the_sealed_ones": (
                    ...
                    "comparison needs, and neither is comparable to a S21D5-025 seal"
                ),
            },
